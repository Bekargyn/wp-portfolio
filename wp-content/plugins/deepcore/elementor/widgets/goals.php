...
class Webnus_Element_Widgets_Goal extends \Elementor\Widget_Base {
		...
		while ( $query->have_posts() ) :
			...
			if ( $type == 'grid' ) {
				...
				<ul class="goal-metadata">
					...
					<li  class="goal-views"> <i class="fa-eye"></i><span><?php echo deep_getViews( $post_id ); ?></span><?php esc_html_e( ' Views', 'deep' ); ?></li>
				</ul>
				</div>
					<?php
					echo $content . '<div class="goal-meta">' . $progressbar;
					if ( $days_left >= 0 && $percentage < 100 && $deep_options['deep_webnus_donate_form'] ) {
						echo deep_modal_donate();
					} else {
						echo '<p class="goal-completed">' . esc_html__( 'Has been completed', 'deep' ) . '</p>';

 ...
 * @since   1.0.0
 */
if ( ! get_option( 'deepPreviousVersion' ) ) {
	update_option( 'deepPreviousVersion', str_replace( '.', '', DEEP_VERSION ) );
	update_option( 'deepCurrentVersion', str_replace( '.', '', DEEP_VERSION ) );
}
...
function deep_theme_setup() {
	...
	// add_action( 'init', 'deep_prebuilds_header' );
	...
	add_action( 'wp_enqueue_scripts', 'deep_dyn_header_styles', 999 );
	add_action( 'admin_init', 'kc_section_template' );
...
function deep_dyn_header_styles() {
	wp_enqueue_style( 'header-dyn', DEEP_ASSETS_URL . 'css/frontend/dynamic-style/header.dyn.css', false, rand( 1,100 ) );
}
...
/**
 * King Composer license and section template
 *
 * @author Webnus
 * @since   1.1.1
 */
function kc_section_template() {
	if ( is_plugin_active( 'kingcomposer/kingcomposer.php' ) ) {
		// KC License
		define( 'KC_LICENSE', '93qxqtof-lucq-j66c-e3t8-ow88-hvnxmsl8q7p6' );

		// KC Section
		if ( function_exists( 'kc_prebuilt_template' ) ) {
			$files = glob( get_template_directory() . '/inc/sections-templates/kc/*.xml' );
			foreach ( $files as $file ) :
				$sec_name = str_replace( '-', ' ', basename( $file, '.xml' ) );
				kc_prebuilt_template( $sec_name, $file );
				endforeach;
		}
	}
}
...
function deep_is_blog() {
    return ( is_author() || is_category() || is_tag() || is_date() || is_home() || is_archive() ) && 'post' == get_post_type();
}